        ...
        <div class="container-fluid my-4">
          ...
          <div class="row m-1">
            <div class="card p-3" style="border-radius: 25px">
              <div class="row mb-3">
                <div class="col-6">
                  <div class="h5 font-weight-bold text-black">Kontak</div>
                </div>
                    ...
                    <tr v-for="(item, index) in dataList" :key="item.id">
                      ...
                      <td>
                        ...
                        <button
                          ...
                          @click="konfirmasi(item.id, item.kontak)"
                        >
  ...
  methods: {
    ...
    async sendUpdateData() {
      try {
        ...
        formData.append("kontak", this.dataUpdate.kontak);
        // formData.append("isi", this.dataUpdate.isi);
        // formData.append("footer", this.dataUpdate.footer);
        const response = await axios.post(
          `${import.meta.env.VITE_API_ENDPOINT}/kontak/update/${
            this.dataUpdate.id
          }`,
          formData,
          {
    ...
    async tambahData() {
      try {
        const formData = new FormData();
        formData.append("kontak", this.dataCreate.kontak);
        // formData.append("isi", this.dataCreate.isi);
        // formData.append("footer", this.dataCreate.footer);
        const response = await axios.post(
          `${import.meta.env.VITE_API_ENDPOINT}/kontak/create`,
          formData,
          {
            headers: {
              Authorization: `Bearer ${sessionStorage.getItem("token")}`,
    ...
    async fetchData() {
      ...
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_ENDPOINT}/kontak/list`,
          {
            headers: {
              Authorization: `Bearer ${sessionStorage.getItem("token")}`,
            },
    ...
    async deleteData(id) {
      try {
        const response = await axios.delete(
          `${import.meta.env.VITE_API_ENDPOINT}/kontak/delete/${id}`,
          {
            headers: {
              Authorization: `Bearer ${sessionStorage.getItem("token")}`,
            },
